...
DLLEXPORT(void) flClose(struct FLContext *handle) {
	if ( handle ) {
		...
		while ( queueDepth-- ) {
			uStatus = usbBulkAwaitCompletion(handle->device, &completionReport, NULL, UINT_MAX);
		}
	...
	while ( count > spaceAvailable ) {
		...
		while ( queueDepth > 2 && !handle->completionReport.flags.isRead ) {
			uStatus = usbBulkAwaitCompletion(
				handle->device, &handle->completionReport, error, UINT_MAX);
	...
	const char **error, long tim)
{
	...
	if ( count >= 0x10000 ) {
		fStatus = flReadChannelAsyncSubmit(handle, chan, 0x10000, buffer, error, tim);
		...
		while ( count >= 0x10000 ) {
			...
			count -= 0x10000;
			buffer += 0x10000;
			fStatus = flReadChannelAsyncAwait(handle, &data, &requestLength, &actualLength, error, tim);
			CHECK_STATUS(fStatus, fStatus, cleanup, "flReadChannel()");
			CHECK_STATUS(
		...
		if ( count ) {
			fStatus = flReadChannelAsyncSubmit(handle, chan, (uint32)count, buffer, error, tim);
			CHECK_STATUS(fStatus, fStatus, cleanup, "flReadChannel()");
			fStatus = flReadChannelAsyncAwait(handle, &data, &requestLength, &actualLength, error, tim);
			CHECK_STATUS(fStatus, fStatus, cleanup, "flReadChannel()");
			CHECK_STATUS(
...
DLLEXPORT(FLStatus) flReadChannelAsyncSubmit(
	struct FLContext *handle, uint8 chan, uint32 count, uint8 *buffer, const char **error, long tim)
{
	FLStatus retVal = FL_SUCCESS, fStatus;
	uint8 command[3];
	USBStatus uStatus;
	...
	const char **error, long tim)
{
	...
	while ( !handle->completionReport.flags.isRead ) {
		uStatus = usbBulkAwaitCompletion(
			handle->device, &handle->completionReport, error, tim
		);
